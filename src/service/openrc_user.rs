...
pub fn install() -> Result<()> {
    ...
    run(&["rc-update", "--user", "add", "bthman", "default"])?;
    info!("Enabled bthman (user). Start with: rc-service --user bthman start");
    Ok(())
}
...
pub fn uninstall() -> Result<()> {
    ...
    let _ = run(&["rc-service", "--user", "bthman", "stop"]);
    let _ = run(&["rc-update", "--user", "del", "bthman", "default"]);
    let init_path = user_init_dir()?.join("bthman");
    if init_path.exists() {
